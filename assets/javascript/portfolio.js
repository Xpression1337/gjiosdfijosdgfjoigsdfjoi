...
const ipgeolocation = 'https://api.ipgeolocation.io/ipgeo?apiKey=b90e7da3a73244448f18dab3191dc2af';
...
$(document).ready(() => {
  const links = [
    {
      name: 'CLICK TO BUY',
      link: 'https://discord.com/users/831549591232970813',
    },
    {
      name: 'CLICK TO BUY',
      link: 'https://discord.com/users/831549591232970813',
  ...
  app.titleChanger(['Xpression#1337', 'Scarlett-Hook']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Checking Valorant...', "Granting access to <span style='font-size: 14px; color: #06d;'>[Scarlett-Hook]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[Injected]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      if (app.skippedIntro) return;

      clearCursor();

      writeLine([`<i style='color: #F62459'>Xpression#1337 $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
